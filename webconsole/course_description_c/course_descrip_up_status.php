<?php
	...
	$udescription_type = isset($_REQUEST['udescription_type'])?$_REQUEST['udescription_type']:"";
	$upost_type = isset($_REQUEST['upost_type'])?$_REQUEST['upost_type']:"";

	$uproduct_descrips = isset($_REQUEST['uproduct_descrips'])?$_REQUEST['uproduct_descrips']:"";
	$media_content_hentities = htmlentities($uproduct_descrips);
	...
	$update_data = array(  
           	...
           	"product_type" => $udescription_type,
			"categories_group" => $upost_type,
			"product_description" => $uproduct_descrips,
            'update_user' => $social_Upost_Id,
            'status' => $social_Status_select,
            'update_time' => $currentdates  
      );  
      $where_condition = array(  
           'product_id' => $articlesId  
      );  
      if(update("tbl_product_description", $update_data, $where_condition))  
      {  
           //echo 'good';
       
      $selectCatebasedRecord = selectCatebasedRecord('tbl_product_description','courses');
	  ...
	  $selectCatebasedRecord_json_count = isset($selectCatebasedRecord_json['selectCatebasedRecord_count'])?$selectCatebasedRecord_json['selectCatebasedRecord_count']:"";
		...
		foreach ($selectCatebasedRecord_json['selectCatebasedRecord_details'] as $navmultiple_details) {
			$nav_id = $navmultiple_details["product_id"];
