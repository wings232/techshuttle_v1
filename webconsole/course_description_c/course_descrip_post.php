<?php
	...
	echo $parent_val = isset($_REQUEST['parent_val'])?$_REQUEST['parent_val']:"";
	$sub_val = isset($_REQUEST['sub_val'])?$_REQUEST['sub_val']:"";
	$cate_group_val = isset($_REQUEST['cate_group_val'])?$_REQUEST['cate_group_val']:"";
	echo $menu_slug = isset($_REQUEST['menu_slug'])?$_REQUEST['menu_slug']:"";
	...
	$descrip_type = isset($_REQUEST['descrip_type'])?$_REQUEST['descrip_type']:"";
	...
	$product_descrip = isset($_REQUEST['product_descrip'])?$_REQUEST['product_descrip']:"";
	
	$product_descrips = explode("\n", $product_descrip);
    if($product_descrip != ""){
        foreach ($product_descrips as $products_descrips) {
            /*$sql_blogs = "INSERT into blog_about(blog_id,blog_about) values('$blogIds','$blog_abouts')";
            $blog_querys = $conn->query($sql_blogs);*/
            $media_content_hentities = htmlentities($products_descrips);
			...
			if($mediahtml_entity_decode != ""){
				...
				$course_description_insert = array(
					"product_primary_id" => $parent_val,
					"product_pri_slug" => $menu_slug,			
					"product_description" => $mediahtml_entity_decode,
					"product_type" => $descrip_type,
				...
				if(recordsToInsert("tbl_product_description",$course_description_insert)){
